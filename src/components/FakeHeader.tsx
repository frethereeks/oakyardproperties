...
export default function FakeHeader() {
    ...
    return (
        <header ref={headerRef} className={`p-4 fixed w-full h-auto left-0 top-0 z-50 ${fixed ? 'bg-white shadow-md shadow-primary/10' : 'bg-transparent'}`}>
            <div className="container flex justify-end items-center gap-3">
                <Link href={"/"} className='text-[.65rem] md:text-xs text-white'>Members</Link>
                <Link href={"/"} className='text-[.65rem] md:text-xs text-white pl-3 border-l border-l-bluish'>Donations</Link>
            ...
            <div className="md:container mx-auto flex items-center justify-between gap-4 relative">
                <Link href={"/"} className={`uppercase text-white font-bold text-xl md:text-3xl flex items-end`}>CHEDRES<span className='-translate-y-1.5 h-2 w-2 flex rounded-sm bg-pink-500 ml-1'></span></Link>
                <nav className={`absolute md:static z-50 top-full w-full md:w-max flex-1 flex flex-col md:flex-row md:justify-end md:gap-4 bg-bluish md:bg-transparent transition-all duration-300  ${navShow ? 'left-0' : 'left-[100vw]'}`}>
                    ...
                    <Link href={"/tickets"} className='text-sm text-bluish font-bold py-2 px-4 bg-white hover:bg-white/60 hover:text-bluish'>Buy Tickets</Link>
                </nav>
