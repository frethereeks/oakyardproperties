...
export default function StrategicSection() {
    ...
    return (
        <section className='flex flex-col gap-4 md:gap-8 bg-blue/5 py-40 px-4'>
            <AppSlider
            ...
            <div className="container mx-auto">
                <div className="grid md:grid-cols-2 gap-4 md:gap-8 py-10 md:py-10">
                    <div className="relative bg-[#c4c4c4] min-h-64 h-full">
                        <video playsInline src={ASSETS_URL["oakyard_properties_prototype_video"]} poster={ASSETS_URL["pexels-tranmautritam"]} className='w-full h-full absolute top-0 left-0' width={'100%'} controls disablePictureInPicture></video>
                    </div>
                    <div className="flex flex-col gap-6 md:gap-8 md:max-w-lg">
                        {
                            [
                                ...
                                <div key={strategic.id} className="flex flex-col gap-2 md:gap-4">
                                    <Header4 dotted>{strategic.title}</Header4>
